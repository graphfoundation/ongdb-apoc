...
Since APOC relies in some places on ONgDB's internal APIs you need to use the *matching APOC version* for your ONgDB installation.
...
mkdir plugins
pushd plugins
wget https://cdn.graphfoundation.org/apoc/dist/org/neo4j/procedure/apoc/{apoc-release}/apoc-{apoc-release}-all.jar
popd
