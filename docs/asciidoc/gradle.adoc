...
Assume APOC is officially building against 3.2.3 and you want to test drive your build against 3.2.6. You can specify the neo4j version on the commandline:
...
By default, apoc builds against `https://m2.neo4j.org/content/repositories/snapshots`. To instead prefer local artifacts when available, add `mavenLocal()` as the first repo in `build.gradle:46`.
