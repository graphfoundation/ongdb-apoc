...
and the employee's proficiencies on those same required skills as a second vector and use those to calculate a similarity between the employee and the role.
...
Do not use when vectors have only one element, as the similarity computation will always be 1.
