...
Enable `apoc.uuid.enabled=true` in `$ONGDB_HOME/config/neo4j.conf` first.
...
First create a Constraint for the Label and the Property, if you try to add a `uuid` an error occurred.
