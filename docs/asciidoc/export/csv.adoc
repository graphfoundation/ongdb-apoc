...
We may also want to export data into JSON format for importing into other tool sor for general sharing of query results.
...
For a graph containing node properties `age`, `city`, `kids`, `male`, `name`, and `street` and containing relationship properties `bar` and `foo`, we'd have the following: 
...
* <<export-csv-available-procedures, Available Procedures>>
* <<export-csv-file-export, Exporting to a file>>
* <<export-csv-stream-export, Exporting a stream>>
* <<export-csv-examples, Examples>>
    ** <<export-database-csv, Export whole database>>
    ** <<export-nodes-relationships-csv, Export specified nodes and relationships>>
    ** <<export-graph-csv, Export virtual graph>>
    ** <<export-cypher-query-csv, Export results of Cypher query>>
...
|===
include::../../../build/generated-documentation/apoc.export.csv.csv[]
|===
...
The `apoc.export.csv.graph` procedure exports a <<virtual-graph,virtual graph>> to a CSV file or as a stream.
...
The query below creates a virtual graph and stores it in memory with the name `producers.cached` using <<static-values, Static Value Storage>>.
...
|===
| file         | source                        | format | nodes | relationships | properties | time | rows | batchSize | batches | done | data 
| "movies-directed.csv" | "statement: cols(6)" | "csv"  | 0     | 0             | 12         | 3    | 2    | 20000     | 1       | TRUE | NULL
|===
...
|===
| file       | nodes | relationships | properties | data 
| NULL  | 0     | 0             | 12        | "\"name\",\"born\",\"role\",\"title\",\"tagline\",\"released\"
\"Lilly Wachowski\",\"1967\",\"\",\"The Matrix\",\"Welcome to the Real World\",\"1999\"
\"Lana Wachowski\",\"1965\",\"\",\"The Matrix\",\"Welcome to the Real World\",\"1999\"
"
|===
