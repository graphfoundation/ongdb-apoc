...
[abstract]
--
This section describes procedures that can be used to import data from databases that have JDBC support.
--
...
|===
include::../../../build/generated-documentation/apoc.load.csv[lines=1;6..8]
|===
...
Download the https://github.com/adejanovski/cassandra-jdbc-wrapper#installing[Cassandra JDBC Wrapper], and put it into your `$ONGDB_HOME/plugins` directory.
Add this config option to `$ONGDB_HOME/conf/neo4j.conf` to make it easier to interact with the cassandra instance.
...
Put the JDBC driver into the `$ONGDB_HOME/plugins` directory and configure the JDBC-URL in `$ONGDB_HOME/conf/neo4j.conf` with `apoc.jdbc.<alias>.url=<jdbc-url>`
